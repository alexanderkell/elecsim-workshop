...
This paper presents ElecSIM, an open-source ABM that simulates generation companies (GenCos) in a wholesale electricity market. ElecSIM models GenCos as multiple agents and electricity demand as an aggregated agent (which can be expanded to include different types of demand such as industry, household and transport). A power exchange facilitates trades between the two. 

GenCos actively make bids for each of their respective power plants to match demand. Their bids are based on\vphantom{\color{red}on their costs to supply a single unit (1MWh) of electricity, known as} their short run marginal cost (SRMC), which excludes capital and fixed costs. The power exchange links bids \vphantom{\color{red} with priority to the cheapest bids first, also known} using a merit-order dispatch. GenCos invest in power plants based on expected profitability of each plant.	
