...
This paper presents ElecSIM, an open-source ABM that simulates generation companies (GenCos) in a wholesale electricity market. ElecSIM models each GenCo as an independent agent and electricity demand as an aggregated agent (which can be expanded to segmented types of demand). A power exchange facilitates trades between the two. 

GenCos make bids for each of their power plants. Their bids are based on their short run marginal cost (SRMC), which excludes capital and fixed costs. The power exchange accepts bids \vphantom{\color{red} with priority to the cheapest bids first, also known} in merit-order. GenCos invest in power plants based on expected profitability.	
